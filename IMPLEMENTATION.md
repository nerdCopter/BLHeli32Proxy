...
| `protocol/setup_fields.py` | Done, partial by design, **live-verified against real backups on 4 firmware revisions** | Decodes 13 confirmed fields (of the full Setup-block struct) from the decrypted plaintext — every value matches a real BLHeliSuite32xl-produced `.ixi` backup file exactly. Field *names* confirmed stable across 4 different firmware revisions (one confirmed structural change found — see [Setup Block Fields](docs/knowledge/setup-block-fields.md)); byte *offsets* only confirmed on the original reference hardware so far. Remaining fields have no public source (BLHeli_32 is closed-source) and are deliberately left undecoded rather than guessed. Also provides `format_ixi_section()`, a confirmed-fields-only `.ixi`-style section writer — never fabricates unconfirmed fields, and its output must never drive a write-back/restore path (see the module docstring) |
| `cli.py` | Done | `serve`, `gen-cert` (shells out to `openssl`), `dump-setup`/`probe-flash`/`dump-flash` — `--motor-index` drives `protocol/fourwayif.py` (the confirmed-real path); without it, the original direct-adapter `protocol/client.py` path (untested here beyond argument parsing and the archive-write refusal, which is tested). `dump-setup --out FILE` appends a partial-backup `.ixi`-style section via `setup_fields.format_ixi_section()` |
...
`probe-flash` and `dump-flash` CLI commands, backed by
...
2. `dump-flash` refuses at the path level to write into `$BLHELI32PROXY_ARCHIVE_DIR` (the user's
...
- Real-hardware verification of `protocol/` ([PLAN.md §5](PLAN.md#5-implementation-language-and-structure) phase-1-equivalent) if the diagnostic
  `dump-setup` command is ever relied on.
