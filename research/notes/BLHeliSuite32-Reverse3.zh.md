...
![](https://img.elmagnifico.tech/static/upload/elmagnifico/PIbCyT6QjzlvfYG.png)
...
可以看到这里直接取解密后第一个字节作为了
主版本号，也就是0x20，对应字符串的显示其实他是用32
...
同理下一个字符，子版本号，0x3C，对应成字符串显示就是60，实际显示的时他是主.子
然后还去掉了尾巴的0，实际显示出来就是32.6
...
可以看到偏移0x80个字节后的16个字节是ESC
Name，由于我这里是没命名的，所以全空，也就是space
...
偏移0x1C个字节，这个是Note_Config
就是音乐的配置，我这里速度是5间隔是0，所以是 0x50
...
这里不知道为什么，PWM Freq Min和Max
都是0xFF，这个值也不是从解密中读出来的，虽然解密中也有这个内容
...
再结合一下之前的Layout和Rev以及Music
Data，基本上日常需要配置的信息就全了
...
![](https://img.elmagnifico.tech/static/upload/elmagnifico/LBXIxZ35zqDTHMA.png)
...
- <a href="/2021/07/16/BLHeliSuite32-Reverse2/" data-toggle="tooltip"
  data-placement="top" title="BLHeliSuite32逆向（二）">Previous<br />
  <span>BLHeliSuite32逆向（二）</span></a>
- <a href="/2021/07/21/OV-Camera-Compare/" data-toggle="tooltip"
  data-placement="top" title="OV系列摄像头对比">Next<br />
  <span>OV系列摄像头对比</span></a>
